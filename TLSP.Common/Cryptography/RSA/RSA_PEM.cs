...
namespace TLSP.Common.Cryptography {
	...
	public class RSA_PEM {
		...
		/// 通过Java 生成的秘钥初始化RSA
		/// </summary>
		/// <param name="javaKey"></param>
		/// <param name="isPrivate"></param>
		/// <returns></returns>
		static public RSA_PEM FromJavaKey(byte[] javaKey,bool isPrivate = false)
		{
			string pem = isPrivate ? "-----BEGIN RSA PRIVATE KEY-----" : "-----BEGIN PUBLIC KEY-----";
			pem += Convert.ToBase64String(javaKey);
			pem += isPrivate ? "-----END RSA PRIVATE KEY-----" : "-----END PUBLIC KEY-----";
			return FromPEM(pem);
		}

		static public RSA_PEM FromJavaKeyBase64(string javaKey, bool isPrivate = false)
		{
			string pem = isPrivate ? "-----BEGIN RSA PRIVATE KEY-----" : "-----BEGIN PUBLIC KEY-----";
			pem += javaKey;
			pem += isPrivate ? "-----END RSA PRIVATE KEY-----" : "-----END PUBLIC KEY-----";
